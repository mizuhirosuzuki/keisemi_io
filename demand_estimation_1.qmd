...
function addIVColumns!(
    ...
    data[!, Symbol("iv_GH_own_", variable)] = (
        ...
        2 .* data[:, variable] .* (data[:, Symbol(variable, "_sum_own")] .- data[:, variable])
    );
    data[!, Symbol("iv_GH_other_", variable)] = (
        (data[:, :mkt_n] .- data[:, :group_n]) .* data[:, variable].^2 .+ 
        (data[:, Symbol(variable, "_sqr_sum_mkt")] .- data[:, Symbol(variable, "_sqr_sum_own")]) .- 
        2 .* data[:, variable] .* (data[:, Symbol(variable, "_sum_mkt")] .- data[:, Symbol(variable, "_sum_own")])
    );
end
