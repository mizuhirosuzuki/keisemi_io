...
```{julia}
dataHH = CSV.read("data/1_demand_estimation_1/HHsize.csv", DataFrame)
...
```{julia}
dataCPI = CSV.File(
    open(read, "data/1_demand_estimation_1/zni2015s.csv", enc"shift-jis"), 
    select = 1:2,
    skipto = 7
    ) |> DataFrame
...
#| echo: false
#| eval: false
serialize("tmp/1_demand_estimation_2/gmm_res.ser", gmm_res)
```

```{julia}
#| echo: false
gmm_res = deserialize("tmp/1_demand_estimation_2/gmm_res.ser")
```
